...
cd ~/Desktop/Development ; sudo -u $USER mkdir Boot
...
sudo chmod +x /etc/systemd/system/led.service ; sudo systemctl enable led
...
sudo chmod +x /etc/systemd/system/monitor-volume.service ; sudo systemctl enable monitor-volume ; sleep 1 ; clear
